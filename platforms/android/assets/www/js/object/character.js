var Character = function(s, good, type, spawnPos) {
    ...
    if (good) {
        ...
        this.sprite = FLIXI.createSprite("character.good." + (type + 1), this.size, this.size)
    } else {
        if (type <= 0) {
        ...
        this.sprite = FLIXI.createSprite("character.bad." + (type + 1), this.size, this.size)

        this.healthBar = FLIXI.createSprite("color.red", this.size * 2, 20)
        this.healthBar.y -= 30;
